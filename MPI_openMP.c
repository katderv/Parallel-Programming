...
int main(int argc, char *argv[])
{
	...
	// First call MPI_Init
 	...
 	numbers_per_rank = floor(ntrials/n_ranks);
 
 	if( ntrials%n_ranks > 0 ){
 ...
 my_last = my_first + numbers_per_rank;
	...
	#pragma omp parallel shared(ntrials,nvars,rho, epsilon, itermax) private(trial,jj,fx,i,startpt)
	{
	#pragma omp for 
	for (trial = my_first; trial < my_last; trial++) {
